...
Furthermore, An RDF-graph database with a built-in SPARQL Query will also be required to store the RDF output data, perform data/attack analysis, i.e. *attack graph construction and graph queries* as well as to *visualize* the resulting graph. Krystal support RDF triplestore such as [GraphDB](https://graphdb.ontotext.com/), please follow this [installation page](https://graphdb.ontotext.com/documentation/standard/installation.html) for installation. Once the installation  has been completed, GraphDB can be accessed locally via web browser through [http://localhost:7200](http://localhost:7200/). Krystal requires one repository to be created beforehand, please take a look at this [documentation](https://graphdb.ontotext.com/documentation/free/creating-a-repository.html#:~:text=the%20RDF4J%20console.-,Using%20the%20Workbench,Select%20GraphDB%20Free%20repository.).
